...
class TransformerBlock(nn.Module):
    def __init__(self, dimensi_embedding: int = 512, heads: int = 8, faktor_ekspansi: int = 4,
                 dropout: float = 0.2) -> None:
        super(TransformerBlock, self).__init__()

        self.attention = MultiHeadAttention(dimensi_embedding, heads)
        self.norm = nn.LayerNorm(dimensi_embedding)
        self.feed_forward = nn.Sequential(nn.Linear(dimensi_embedding, faktor_ekspansi * dimensi_embedding), nn.ReLU(),
...
class Encoder(nn.Module):
    def __init__(self, panjang_sekuens: int, ukuran_vocab: int, dimensi_embedding: int = 512, jumlah_block: int = 6, faktor_ekspansi: int = 4, heads: int = 8, dropout: float = 0.2) -> None:
        ...
        self.positional_encoding = PositionalEncoding(dimensi_embedding, panjang_sekuens)
